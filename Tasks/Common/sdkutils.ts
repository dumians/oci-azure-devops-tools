...
export abstract class SdkUtils {
    ...
    private static readonly userAgentPrefix: string = 'OCI-VSTS'
    ...
    private static readonly validateUserAgentEnvVariable: string = 'OCIVSTSTesting_ValidateUserAgent'
    ...
    public static setSdkUserAgentFromManifest(taskManifestFilePath: string): void {
        if (fs.existsSync(taskManifestFilePath)) {
            ...
            ;((OCI as any).util as any).userAgent = () => {
    ...
    public static async createAndConfigureSdkClient(
        OCIService: any,
        OCIServiceOpts: any,
        taskParams: OCIConnectionParameters,
        logger: (msg: string) => void
    ): Promise<any> {
        OCIService.prototype.customizeRequests((request: any) => {
            ...
            request.on('complete', (response: any) => {
                try {
                    logger(`OCI ${operation} request ID: ${response.requestId}`)
        ...
        // instance metadata
        if (OCIServiceOpts) {
            if (!OCIServiceOpts.credentials) {
                const creds = await getCredentials(taskParams)
                if (creds) {
                    OCIServiceOpts.credentials = creds
                }
            }
            if (!OCIServiceOpts.region) {
                OCIServiceOpts.region = await getRegion()
            }

            return new OCIService(OCIServiceOpts)
        }

        const credentials = await getCredentials(taskParams)

        return new OCIService({
    ...
    public static async roleArnFromName(iamClient: IAM, roleName: string): Promise<string> {
        ...
        try {
            console.log(`Attempting to retrieve Oracle Resource Name (ARN) for role ${roleName}`)
    ...
    // For tasks that do not use the OCI SDK for Node.js, tests for the preferred
