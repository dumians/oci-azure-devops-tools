...
import { STS } from 'OCI-sdk/clients/all'
import * as OCI from 'OCI-sdk/global'
import * as HttpsProxyAgent from 'https-proxy-agent'
import { format, parse, Url } from 'url'

// Task variable names that can be used to supply the OCI credentials
// to a task (in addition to using a service endpoint, or environment
// variables, or EC2 instance metadata)
export const OCIAccessKeyIdVariable: string = 'OCI.AccessKeyID'
export const OCISecretAccessKeyVariable: string = 'OCI.SecretAccessKey'
export const OCISessionTokenVariable: string = 'OCI.SessionToken'

// Task variable name that can be used to supply the region setting to
// a task.
export const OCIRegionVariable: string = 'OCI.Region'

// default session name to apply to the generated credentials if not overridden
// in the endpoint definition
export const defaultRoleSessionName: string = 'OCI-vsts-tools'
...
export const roleCredentialMaxDurationVariableName: string = 'OCI.rolecredential.maxduration'

export interface OCIConnectionParameters {
    ...
    // fields above
    OCIEndpointAuth: tl.EndpointAuthorization | undefined
}
...
function completeProxySetup(connectionParamaters: OCIConnectionParameters): void {
...
export function buildConnectionParameters(): OCIConnectionParameters {
    const connectionParameters: OCIConnectionParameters = {
...
function attemptEndpointCredentialConfiguration(
    OCIparams: OCIConnectionParameters,
    endpointName: string
): OCI.Credentials | undefined {
    if (!endpointName) {
        return undefined
    }

    OCIparams.OCIEndpointAuth = tl.getEndpointAuthorization(endpointName, false)
    console.log(`...configuring OCI credentials from service endpoint '${endpointName}'`)

    const accessKey = OCIparams.OCIEndpointAuth.parameters.username
    const secretKey = OCIparams.OCIEndpointAuth.parameters.password

    OCIparams.AssumeRoleARN = OCIparams.OCIEndpointAuth.parameters.assumeRoleArn
    if (!OCIparams.AssumeRoleARN) {
        console.log('...endpoint defines standard access/secret key credentials')

        return new OCI.Credentials({
            accessKeyId: accessKey,
            secretAccessKey: secretKey
        })
    }

    console.log(`...endpoint defines role-based credentials for role ${OCIparams.AssumeRoleARN}.`)

    const externalId: string = OCIparams.OCIEndpointAuth.parameters.externalId
    let roleSessionName: string = OCIparams.OCIEndpointAuth.parameters.roleSessionName
    if (!roleSessionName) {
    ...
    const masterCredentials = new OCI.Credentials({
        accessKeyId: accessKey,
        secretAccessKey: secretKey
    })
    const options: STS.AssumeRoleRequest = {
        RoleArn: OCIparams.AssumeRoleARN,
    ...
    return new OCI.TemporaryCredentials(options, masterCredentials)
}
...
// visible as environment vars for the OCI Node.js sdk to auto-recover
// so treat as if a service endpoint had been set and return a credentials
// instance.
function attemptCredentialConfigurationFromVariables(): OCI.Credentials | undefined {
    const accessKey = tl.getVariable(OCIAccessKeyIdVariable)
    if (!accessKey) {
        return undefined
    }

    const secretKey = tl.getVariable(OCISecretAccessKeyVariable)
    if (!secretKey) {
        throw new Error(
            'OCI access key ID present in task variables but secret key value is missing; ' +
                'cannot configure task credentials.'
        )
    }

    const token = tl.getVariable(OCISessionTokenVariable)

    console.log('...configuring OCI credentials from task variables')
...
// can be configured as a service endpoint (of type 'OCI'), or specified
...
export async function getCredentials(OCIParams: OCIConnectionParameters): Promise<OCI.Credentials | undefined> {
    console.log('Configuring credentials for task')

    const credentials =
        attemptEndpointCredentialConfiguration(OCIParams, tl.getInput('OCICredentials', false)) ||
...
async function queryRegionFromMetadata(): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
        const metadataService = new OCI.MetadataService()
...
export async function getRegion(): Promise<string> {
    ...
    region = tl.getVariable(OCIRegionVariable)
    if (region) {
        console.log(`...configured to use region ${region}, defined in task variable ${OCIRegionVariable}.`)
