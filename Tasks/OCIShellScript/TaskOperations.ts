...
export class TaskOperations {
    ...
    // based on the VSTS 'ShellScript' task but modified to inject OCI credentials
    // and region into the environment, and to be able to specify the script inline
    // or from a file
    public async execute(): Promise<number> {
        let scriptPath = ''
        try {
            await this.configureOCIContext()
            await SdkUtils.configureHttpProxyFromAgentProxyConfiguration('OCIShellScript')

            const bash = tool(which('bash', true))

            if (this.taskParameters.scriptType === inlineScriptType) {
                const tempDir = SdkUtils.getTempLocation()
                const fileName = `OCIshellscript_${process.pid}.sh`
    ...
    private async configureOCIContext() {
        ...
        if (region) {
            ...
            // set for the benefit of any OCI cli commands the user might
